...
export const fetchprivacy = async () => {
  const res = await client.getEntries({ content_type: "privacy" });
  return res.items[0];
};
...
export const fetchNews = async (limit, skip, tabname) => {
  const res = await client.getEntries({
    content_type: "news",
    limit,
    skip,
...
export const fetchNewsTabs = async () => {
  const res = await client.getEntries({
    content_type: "newsTabs",
  });
  return { tabs: res.items[0].fields };
};
